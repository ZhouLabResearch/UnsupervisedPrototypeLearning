function net = resnet_customKm_snr(inputSize, numClasses, options)
...
options.InitialPoolingLayer = iValidateInitialPoolingLayer(options.InitialPoolingLayer);
options.ResidualBlockType   = iValidateResidualBlockType(options.ResidualBlockType);
options.BottleneckType      = iValidateBottleneckType(options.BottleneckType);
options.Normalization       = iValidateNormalization(options.Normalization);
...
function x = iValidateInitialPoolingLayer(x)
x = validatestring(x, {'none','max','average'});
end

function x = iValidateResidualBlockType(x)
x = validatestring(x, {'batchnorm-before-add','batchnorm-after-add'});
end

function x = iValidateBottleneckType(x)
x = validatestring(x, {'none','downsample-first-conv'});
end

function x = iValidateNormalization(x)
x = validatestring(x, {'zerocenter','zscore'});
end
...
%%% prototype
%     fullyConnectedLayer(256,'Name','op','WeightsInitializer', iDefaultWeightsInitializer()),...
%     fullyConnectedLayer(128,'Name','p','WeightsInitializer', iDefaultWeightsInitializer()),...
%     fullyConnectedLayer(64,'Name','pg','WeightsInitializer', iDefaultWeightsInitializer()),...
...
%     eluLayer,...
%     fullyConnectedLayer(64,'Name','pg','WeightsInitializer', iDefaultWeightsInitializer()),...
%     fullyConnectedLayer(numClasses,'Name','ggap','WeightsInitializer', iDefaultWeightsInitializer()),...
%     customRegressionLayerKm() ];
% end
function net = iFinalLayers(numClasses)
net = [
    globalAveragePooling2dLayer("Name","gap"),...
...
switch residualBlockType
    ...
    otherwise
        error(message('nnet_cnn:resnetLayers:InvalidResidualBlockType'));
end
end
